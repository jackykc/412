...
class VisualServo(smach.State):
    ...
    def execute(self, data):
        ...
        while not rospy.is_shutdown():
            # print current_marker_pose
            if (current_marker_pose is not None) and (current_marker_pose.position.x > 0.7):
...
class NavigateGoal(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes=['start', 'finish'])
        self.twist = Twist()
        print "init"
    def execute(self, data):
        global current_marker_pose
